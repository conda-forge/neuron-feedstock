...
if [ `uname` == Darwin ]; then
    if [ "$PY_VER" == "3.8" ]; then
        pip install https://files.pythonhosted.org/packages/3a/8a/ca66dc5cf5f49237ae2c854b4f5b179786a8e55c64aac4a757a2a13f3193/NEURON-7.8.1.1-cp38-cp38-macosx_10_9_x86_64.whl --no-deps -v
    else
...
if [ `uname` == Linux ]; then
    if [ "$PY_VER" == "3.8" ]; then
        pip install https://files.pythonhosted.org/packages/ef/73/2bda5b486fda5a8e437331bba76b431fd58c9cea5ff80e73e64646b45523/NEURON-7.8.1.1-cp38-cp38-manylinux1_x86_64.whl --no-deps -v
    else
        pip install NEURON --no-deps -v
    fi
fi
